...
class la_vector {
    ...
    static_assert(t_bpc <= sizeof(K) * CHAR_BIT, "t_bpc must be less than or equal to the number of bits in K");
    struct segment;
    struct constant_bpc;
    struct variable_bpc;
    class la_iterator;
...
struct la_vector<K, t_bpc, t_top_level>::segment : base_segment_type {
    ...
    segment(const canonical_segment &cs, uint8_t bpc, position_type corrections_offset,
        ...
        for (auto k = i; k < j; k++) {
            ...
            if (BIT_WIDTH(correction) > this->bpc) {
                std::cout << "data: " << (int64_t) data[k] << "approx: " << (int64_t) approximate(k) << "err " << (int64_t) error << "first: " << first << " slope_e" << (int) slope_exponent << " slope_s" << slope_significand << " intercept" << intercept << std::endl;
            }
    ...
    void set_correction(uint64_t *corrections, size_t n, size_t i, uint64_t value) {
        ...
        //if (BIT_WIDTH(value) > this->bpc)
          //  throw std::overflow_error(std::format("Segment correction too large on index {} with value {}.", i, value));
        if (i == first)
